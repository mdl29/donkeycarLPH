<template>
  <div class="header">
    ...
    <div class="count">
      <img class="groupIcon" src="../assets/group.svg" />
      {{ userCount }}
    </div>
  </div>
  <div class="jobs-card">
    ...
    <div class="running-jobs">
      <runningJobCard
        ...
        :job="job"
        :reload="reload"
        :carColor="getCarColor(job.worker_id)"
        :carName="getCarName(job.worker_id)"
        @resume="resumeJob($event)"
        @reload="reloadJob($event)"
      ...
      <span
        class="no-job"
        v-if="runningJobs.length === 0 && waitingJobs.length === 0 && cars.length !== 0"
      >
        No running job
      </span>
    </div>
