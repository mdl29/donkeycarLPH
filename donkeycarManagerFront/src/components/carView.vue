<template>
  <div class="car">
    <div class="header" :class="{ blur: is_paused }">
      <div
        class="record"
        v-if="job && (job.name === 'RECORD' || job.name === 'AUTO')"
      >
        {{ job.name === "RECORD" ? "REC" : "AUTO" }}
      </div>
      <div class="username" v-if="job">{{ job.player.player_pseudo }}</div>
      <div :class="{ name_job: job, name: !job }" :style="{ '--color': color }">
        {{ car.name }}
      </div>
    </div>
    <div class="content" :class="{ blur: is_paused }">
      <div v-if="job" class="job">
        <div v-if="race" class="race">
          ...
          <div
            class="lap head"
            v-if="race.laptimers.length > 0 || timeleft > 0"
          >
...
export default {
  ...
  mounted () {
    ...
    this.interval = setInterval(() => {
      if (that.race) {
        that.timeleft =
          that.race.max_duration * 1000 -
          that.fromNow(that.race.start_datetime)
        const start =
