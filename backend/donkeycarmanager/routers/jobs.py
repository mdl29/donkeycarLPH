...
async def create_job(job: schemas.Job,
                     db: Session = Depends(get_db)):
    # TODO check job_id isn't already in the waiting queue
    return await crud.create_job(db=db, job=job)


@router.get("/", response_model=List[schemas.Job])
def read_job(by_rank: bool = True, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_jobs(db, skip=skip, by_rank=by_rank, limit=limit)
...
async def move_job_after(
    ...
    return await crud.move_job_after_another_in_queue(
        db=db, to_move=to_move, before_item=before_item)
...
async def move_job_before(
    ...
    return await crud.move_job_before_another_queue(
        db=db, to_move=to_move, after_item=after_item)
